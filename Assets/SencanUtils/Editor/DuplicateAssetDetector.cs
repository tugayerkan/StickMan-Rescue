...
public class DuplicateAssetDetector : EditorWindow, IHasCustomMenu
{
	...
	[MenuItem("Utils/SencanUtils/Duplicate Asset Detector")]
	...
	private void SaveSession( string json )
	{
		...
		SaveSystem.Save("DuplicateAssetDetector", json, SaveSystem.SaveType.TextFile,
			SaveSystem.SavePathType.Editor_Erasable);
	}
	...
	private void LoadSession( string json )
	{
		if( string.IsNullOrEmpty( json ) )
		{
			if( !SaveSystem.HasSaveFile("DuplicateAssetDetector", SaveSystem.SaveType.TextFile, SaveSystem.SavePathType.Editor_Erasable))
				return;

			json = SaveSystem.Load<string>("DuplicateAssetDetector", SaveSystem.SaveType.TextFile,
				SaveSystem.SavePathType.Editor_Erasable);
		}
